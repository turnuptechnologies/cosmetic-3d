...
export function HeroSectionAnimation({
  ...
  return (
    <div className='scroll-smooth bg-black no-scrollbar overflow-x-hidden' style={{ zIndex: -9999, pointerEvents: 'all' }}>

      {/* ================= 3D MODEL CONTAINER ================= */}
      <div ref={modalRef} className="will-change-transform" style={{ pointerEvents: 'none' }} aria-hidden="true">
      ...
      <section ref={section1Ref} className="relative h-screen w-full flex items-center justify-center overflow-hidden bg-black mb-10">
        ...
        {[1, 2].map((layer) => (
          <div key={layer} aria-hidden={layer === 2 ? true : undefined} className={`absolute inset-0 flex items-center justify-center w-full pt-1 sm:pt-0 ${layer === 1 ? 'z-10' : 'z-30'}`}>
            <motion.h1
        ...
        <div ref={bottomLeftRef} className="absolute bottom-10 left-4 sm:left-16 lg:left-24 max-w-md z-40 space-y-6">
          ...
          <Link
            href="/contact"
            className="pointer-events-auto group inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-[#FF4F7A] to-pink-600 text-white rounded-full font-semibold hover:shadow-lg hover:shadow-pink-500/50 transition-all"
          >
            <span>{ctaLabel}</span>
            <div className="bg-white rounded-full p-1"><motion.svg className="w-4 h-4" fill="none" stroke="#ff4f7a" viewBox="0 0 24 24" aria-hidden="true" style={{ rotate: -35 }}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" /></motion.svg></div>
          </Link>
        </div>
{/* bottom-38 sm:bottom-38 md:bottom-41 right-10 */}
      ...
      <section ref={section2Ref} className="flex h-screen w-full items-center px-16 bg-black text-white relative">
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-24 items-center w-full">
                ...
                <div key={index} className="flex items-start gap-4 p-2 pointer-events-auto">
                  <div className="p-2 rounded-full text-[#CBFF00]">
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                      ...
                      {index === 4 && <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" />}
          ...
          <div aria-hidden="true" className="h-screen bg-no-repeat bg-center bg-contain opacity-50 lg:opacity-100" style={{ backgroundImage: "url('/images/11.png')" }} />
        </div>
      </section>

      {/* ================= SECTION 3 ================= */}
      <div ref={section3Ref} aria-hidden="true" />
    </div>
  );
}
